...
class PipelineManager:
    ...
    def _init_db(self):
        with sqlite3.connect(self.db_path) as conn:
            ...
            # Alter table if columns missing (for existing DBs)
            try: cursor.execute("ALTER TABLE task ADD COLUMN stage TEXT")
            except sqlite3.OperationalError: pass
            try: cursor.execute("ALTER TABLE task ADD COLUMN table_name TEXT")
            except sqlite3.OperationalError: pass
            try: cursor.execute("ALTER TABLE task ADD COLUMN run_id TEXT")
            except sqlite3.OperationalError: pass
    ...
    def _calculate_next_run(self, run_ts, frequency):
        ...
        if frequency == "daily": return run_dt + timedelta(days=1)
        if frequency == "weekly": return run_dt + timedelta(days=7)
        if frequency == "hourly": return run_dt + timedelta(hours=1)
        if frequency == "monthly": return run_dt + timedelta(days=30)
        return None
    ...
    def add_or_get_schedule(self, source_id, schedule_name,
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT schedule_id FROM schedule WHERE schedule_name=?", (schedule_name,))
            row = cursor.fetchone()
            if row: return row[0]
            next_run = None
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (source_id, schedule_name, frequency, run_ts, next_run, timezone,
                  priority_nbr, active_flag, datetime.now(), "system"))
            conn.commit()
            return cursor.lastrowid
    ...
    def add_batch(self, schedule_id, batch_name, batch_type,
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, (schedule_id, batch_name, batch_type,
                  priority_nbr, active_flag, datetime.now().date(),
                  datetime.now(), "system"))
            conn.commit()
            return cursor.lastrowid
    ...
    def start_task(self, source_id, target_id, schedule_id, batch_id, task_name,
                   pipeline_run_id, table_name=None, stage=None):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute("""
                INSERT INTO task (
                    ...
                    task_name, run_id, status, stage, table_name,
                    start_ts, insert_ts, insert_user
                )
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (pipeline_run_id, source_id, target_id, schedule_id, batch_id,
                  task_name, run_id, "running", stage, table_name,
    ...
    def complete_task(self, run_id):
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute("""
                ...
                WHERE run_id=?
            """, (end_dt, duration, datetime.now(), "system", run_id))
            conn.commit()
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute("""
                ...
                SET status='failed', end_ts=?, error_msg=?,
                    update_ts=?, update_user=?
                WHERE run_id=?
            """, (end_dt, error_msg, datetime.now(), "system", run_id))
            conn.commit()
